...
-- DB - extend tech tooltip to include bonus [YT_LFOC]
		if GameConfiguration.GetValue("YT_LEARN_FROM_OTHER_CIVS") then
			...
			if DB_YT.ProspectInformation ~= nil then
				Bonus, Note, _ = DB_YT.ProspectInformation(ePlayer, playerTechnology:GetResearchingTech(), "S")
			end
...
-- DB - extend culture tooltip to include bonus [YT_LFOC]
		if GameConfiguration.GetValue("YT_LEARN_FROM_OTHER_CIVS") then
			...
			local Note = ""
			if DB_YT.ProspectInformation ~= nil then
				Bonus, Note, _ = DB_YT.ProspectInformation(ePlayer, playerCulture:GetProgressingCivic(), "C")
			end
...
-- DB - expand tourism tooltip [BR]
		if Modding.IsModActive('238f9daf-7d74-429b-84b9-564ca3e79ac7') then
			...
			tourismRateTT = tourismRateTT .. "[NEWLINE]====================================================="
			-- Domestic Tourists: 153
			tourismRateTT = tourismRateTT .. "[NEWLINE]" .. Locale.Lookup("LOC_BR_MY_DOMESTIC_TOURISTS", string.format("%d", LocalPlayerCulture:GetStaycationers()))
			...
			tourismRateTT = tourismRateTT .. "[NEWLINE][NEWLINE]" .. Locale.Lookup("LOC_BR_CULTURE_VICTORY") .. Locale.Lookup("LOC_BR_LABEL")
		end
-- /DB
