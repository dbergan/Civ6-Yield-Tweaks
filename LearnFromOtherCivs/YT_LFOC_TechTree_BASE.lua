...
function PopulateNode(uiNode, playerTechData)
	...
	for _, Player in pairs(AllPlayers) do
		...
		local HasTech = Player:GetTechs():HasTech(GameInfo.Technologies[uiNode.Type].Index)
	...
	if DB_YT.ProspectInformation ~= nil then
		local Turns = 0
		_, _, Turns = DB_YT.ProspectInformation(LocalPlayerID, GameInfo.Technologies[uiNode.Type].Index, "S")
		if Turns <= 0 then
